...
`/aidd-riteway-ai` teaches agents how to write isolated `riteway ai` prompt
evals (`.sudo` files) for multi-step agent flows, lifecycle gates, and evidence
claims.
...
8. Adds lifecycle scenarios that reject unrun baselines, failed required gates,
   missing evidence, silent scope expansion, and false readiness claims
